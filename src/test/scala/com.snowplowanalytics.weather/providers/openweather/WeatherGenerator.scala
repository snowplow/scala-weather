...
trait WeatherGenerator {
  ...
  def genWeatherDescription: Gen[List[WeatherCondition]] = for {
  ...
  def genWeatherStamp: Gen[Weather] = for {
    main <- genMain
    wind <- genWind
    clouds <- genClouds
  ...
  def genEmptyHistoryBatch: Gen[History] = for {
    cnt <- Gen.choose(1,23)
    wind <- genWind
    count <- cnt
  } yield History(cnt, "200", Nil)

  def genNonEmptyHistoryBatch: Gen[History] = for {
    cnt <- Gen.choose(2,20)
    wind <- genWind
    seed <- Gen.choose(-1, 3)
    count <- cnt
  ...
  /**
    ...
    */
  def genPredefinedPosition(positions: Vector[Position]): Gen[Position] = for {
