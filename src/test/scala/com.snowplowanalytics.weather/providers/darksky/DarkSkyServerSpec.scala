...
class DarkSkyServerSpec(val env: Env) extends Specification with ScalaCheck with OwnExecutionEnv {
  ...
  def e5 = {
    val ioClient = CreateDarkSky[IO].create("api.darksky.net/forecast", "INVALID-KEY", 1.seconds)
    ...
    val evalClient = CreateDarkSky[Eval].create("api.darksky.net/forecast", "INVALID-KEY", 1.seconds)
